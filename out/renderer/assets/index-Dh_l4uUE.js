...
const buttons$1 = { "browse": "Browse", "loadMosque": "Load Mosque", "refresh": "Refresh", "advanced": "Advanced...", "customize": "Customize", "save": "Save", "cancel": "Cancel", "generatePng": "Generate PNG", "generateXlsx": "Generate XLSX", "resetDefaults": "Reset to defaults", "switchToEnglish": "English", "switchToTurkish": "Türkçe" };
...
const mosqueModal$2 = { "title": "Load Mosque", "availableTitle": "Available Mosques", "addTitle": "Add Mosque", "buttons": { "load": "Load", "add": "Add", "showAdd": "Add Mosque", "hideAdd": "Hide Add Mosque", "delete": "Delete Mosque" }, "messages": { "added": "Mosque added.", "deleted": "Mosque deleted." }, "errors": { "selectMosque": "Select a mosque to load.", "requiredFields": "Location, mosque name, and address are required.", "duplicate": "A mosque with this location and address already exists.", "deleteBuiltin": "Built-in mosques cannot be deleted." }, "confirm": { "delete": 'Delete mosque "{{name}}"?' } };
...
const buttons = { "browse": "Gözat", "loadMosque": "Cami yükle", "refresh": "Yenile", "advanced": "Gelişmiş...", "customize": "Özelleştir", "save": "Kaydet", "cancel": "İptal", "generatePng": "PNG üret", "generateXlsx": "XLSX üret", "resetDefaults": "Varsayılanlara dön", "switchToEnglish": "English", "switchToTurkish": "Türkçe" };
...
const mosqueModal$1 = { "title": "Cami yükle", "availableTitle": "Mevcut Camiler", "addTitle": "Cami Ekle", "buttons": { "load": "Yükle", "add": "Ekle", "showAdd": "Cami Ekle", "hideAdd": "Cami Eklemeyi Gizle", "delete": "Camiyi Sil" }, "messages": { "added": "Cami eklendi.", "deleted": "Cami silindi." }, "errors": { "selectMosque": "Yüklemek için bir cami seçin.", "requiredFields": "Konum, cami adı ve adres zorunludur.", "duplicate": "Bu konum ve adres için cami zaten var.", "deleteBuiltin": "Yerleşik camiler silinemez." }, "confirm": { "delete": '"{{name}}" camisini silmek istiyor musunuz?' } };
const tr = {
  app,
  labels,
  buttons,
...
const loadMosqueButton = getEl("loadMosque");
const masjidAddressInput = getEl("masjidAddress");
const mosqueModal = getEl("mosqueModal");
const mosqueListSelect = getEl("mosqueListSelect");
const loadSelectedMosqueButton = getEl("loadSelectedMosque");
const closeMosqueModalButton = getEl("closeMosqueModal");
const deleteSelectedMosqueButton = getEl("deleteSelectedMosque");
const mosqueAddPane = getEl("mosqueAddPane");
const addMosqueButton = getEl("addMosque");
const mosqueModalCountrySelect = getEl("mosqueModalCountry");
const mosqueModalStateSelect = getEl("mosqueModalState");
const mosqueModalCitySelect = getEl("mosqueModalCity");
const mosqueModalNameInput = getEl("mosqueModalName");
const mosqueModalAddressInput = getEl("mosqueModalAddress");
const mosqueModalMessageEl = getEl("mosqueModalMessage");
...
let customMosquesState = [];
let selectedMosqueId = BUILTIN_MOSQUE_ID_DEFAULT;
let mosqueModalLocationState = { ...DEFAULT_LOCATION_SELECTION };
void bootstrap();
...
function isRecordAppliedToForm(record) {
...
function resolveSelectedMosqueId(candidate) {
  if (candidate && getMosqueById(candidate)) {
    return candidate;
  }
  return BUILTIN_MOSQUE_ID_DEFAULT;
}
function syncSelectedMosqueFromCurrentFields() {
  const location = getSelectedLocationSelection();
  const address = normalizeForCompare(masjidAddressInput.value);
  const name = normalizeForCompare(masjidNameInput.value);
  const found = getAllMosques().find(
    (entry) => isSameLocation(entry.location, location) && normalizeForCompare(entry.address) === address && (entry.kind === "builtin" || normalizeForCompare(entry.name) === name)
  );
...
function sanitizeCustomMosques(items) {
  ...
  for (const rawItem of items) {
    ...
    if (locationInput.countryId !== location.countryId || locationInput.stateId !== location.stateId || locationInput.cityId !== location.cityId || seen.has(id)) {
      continue;
    }
    out.push({
      kind: "custom",
...
function saveCustomMosques(items = customMosquesState) {
...
function makeCustomMosqueId(name, location) {
  const base = `${name}-${location.cityId}`.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "mosque";
...
function getMosqueOptionLabel(entry) {
  const labels2 = getLocationLabels(entry.location);
  return `${getMosqueDisplayName(entry)} (${labels2.city}, ${labels2.state}, ${labels2.country})`;
}
function renderMosqueList(selectedId = selectedMosqueId) {
  ...
  mosqueListSelect.value = records.some((entry) => entry.id === resolved) ? resolved : BUILTIN_MOSQUE_ID_DEFAULT;
  selectedMosqueId = mosqueListSelect.value || BUILTIN_MOSQUE_ID_DEFAULT;
}
function setMosqueModalMessage(message, isError = false) {
...
function setMosqueAddPaneVisible(visible) {
  mosqueAddPane.open = visible;
}
function syncMosqueModalLocationSelectors(partial) {
  const normalized = normalizeLocationSelection(partial);
  renderSelectOptions(
...
function getMosqueModalLocationSelection() {
  mosqueModalLocationState = normalizeLocationSelection({
    countryId: mosqueModalCountrySelect.value,
    stateId: mosqueModalStateSelect.value,
    cityId: mosqueModalCitySelect.value
  });
  return mosqueModalLocationState;
}
async function applyMosqueRecord(record, shouldRefreshMonths) {
...
function addCustomMosqueFromModal() {
  ...
  const duplicate = getAllMosques().find(
    (entry) => isSameLocation(entry.location, location) && normalizeForCompare(entry.address) === normalizeForCompare(address)
  );
  if (duplicate) {
...
function deleteSelectedMosqueFromModal() {
  const selected = getMosqueById(mosqueListSelect.value);
  if (!selected) {
    setMosqueModalMessage(t("mosqueModal.errors.selectMosque"), true);
    return;
...
function bindMosqueModalHandlers() {
...
function parseV5Entries(raw) {
  try {
    ...
    return {
      ...
      selectedMosqueId: typeof parsed.selectedMosqueId === "string" ? parsed.selectedMosqueId : void 0
    };
  } catch {
    return null;
  }
